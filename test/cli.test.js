...
const { run, HELP_TEXT } = require("../src/cli");
...
test("prints help when no command is provided", async () => {
  ...
  const exitCode = await run([], { stdout, stderr, robot: createRobot() });
...
test("moveMouse uses smooth movement internally", async () => {
  ...
  const exitCode = await run(["moveMouse", "450", "890"], { stdout, stderr, robot });
...
test("click uses the native default signature when no button is supplied", async () => {
  ...
  const exitCode = await run(["click"], { stdout, stderr, robot });
  const result = JSON.parse(stdout.read());
...
test("click with coordinates moves smoothly before clicking", async () => {
  ...
  const exitCode = await run(["click", "10", "20"], { stdout, stderr, robot });
  const result = JSON.parse(stdout.read());
...
test("types joined text with an optional typing speed", async () => {
  ...
  const exitCode = await run(["type", "hello", "world", "--cpm", "300"], { stdout, stderr, robot });
  const result = JSON.parse(stdout.read());
...
test("captures a screenshot and saves it to the requested output path", async () => {
  ...
  const exitCode = await run(
    ["screenshot", "10", "20", "30", "40", "--output", "screen.bmp"],
    { stdout, stderr, robot, cwd }
  );
  const result = JSON.parse(stdout.read());
...
test("findImage resolves the image path and returns screen coordinates", async () => {
  ...
  const exitCode = await run(
    ["findImage", "assets/button.bmp", "--x", "100", "--y", "200", "--width", "300", "--height", "400", "--tolerance", "0.2"],
    { stdout, stderr, robot, cwd }
  );
  const result = JSON.parse(stdout.read());
...
test("clickImage reports not-found without failing the process", async () => {
  ...
  const exitCode = await run(["clickImage", "button.bmp"], {
    stdout,
    stderr,
    robot,
...
test("clickWord uses OCR matches and moves the mouse smoothly to the selected text", async () => {
  ...
  const exitCode = await run(["clickWord", "continue"], {
    stdout,
    stderr,
    robot,
    ocrBackend: {
...
test("findText reads a BMP capture into an ArrayBuffer for OCR", async () => {
  ...
  const exitCode = await run(["findText", "Message"], {
    stdout,
    stderr,
    robot,
    ocrBackend: {
...
test("clickText scans multiple displays and clicks matches found on a secondary display", async () => {
  ...
  const exitCode = await run(["clickText", "Continue"], {
    stdout,
    stderr,
    robot,
    ocrBackend: {
...
test("findText keeps explicit bounds instead of scanning every display", async () => {
  ...
  const exitCode = await run(["findText", "Message", "--x", "10", "--y", "20", "--width", "30", "--height", "40"], {
    stdout,
    stderr,
    robot,
    ocrBackend: {
...
test("findText keeps the no-arg capture fast path when only one display is active", async () => {
  ...
  const exitCode = await run(["findText", "Continue"], {
    stdout,
    stderr,
    robot,
    ocrBackend: {
...
test("returns a non-zero exit code and error JSON for invalid commands", async () => {
  const stdout = createStream();
  const stderr = createStream();
  const exitCode = await run(["dance"], { stdout, stderr, robot: createRobot() });
  const result = JSON.parse(stderr.read());
...
test("moveMouseSmooth is no longer a public CLI command", async () => {
  const stdout = createStream();
  const stderr = createStream();
  const exitCode = await run(["moveMouseSmooth", "1", "2"], { stdout, stderr, robot: createRobot() });
  const result = JSON.parse(stderr.read());
...
test("external OCR override receives the BMP capture path and preserves legacy output", async () => {
  ...
  const exitCode = await run(
    ["findText", "Legacy", "--ocr", "./fake-ocr", "--rec-langs", "en-US"],
    {
      stdout,
      stderr,
      robot,
...
test("openApp delegates an arbitrary application name to the platform", async () => {
  ...
  const exitCode = await run(["openApp", "Example", "App"], {
    stdout,
    stderr,
    platform: "darwin",
    runProcess(command, args, label) {
...
test("activateApp delegates an arbitrary application name to the platform", async () => {
  ...
  const exitCode = await run(["activateApp", "Example", "App"], {
    stdout,
    stderr,
    platform: "darwin",
    runProcess(command, args, label) {
...
test("waitForText performs the capture and retry loop in one command", async () => {
  ...
  const exitCode = await run(["waitForText", "Ready", "--timeout", "1000"], {
    stdout,
    stderr,
    robot,
    ocrBackend: {
...
test("waitForImage loads the target once and retries screen observations locally", async () => {
  ...
  const exitCode = await run(["waitForImage", "button.bmp", "--timeout", "1000"], {
    stdout,
    stderr,
    robot,
    now: () => clock,
...
test("waitForImage stops after its local timeout", async () => {
  ...
  const exitCode = await run(["waitForImage", "button.bmp", "--timeout", "200"], {
    stdout,
    stderr,
    robot,
    now: () => clock,
...
test("findText returns every instance and clickText can select the second one positionally", async () => {
  ...
  const findExitCode = await run(["findText", "Message General"], {
    stdout: findStdout,
    stderr: findStderr,
    robot,
    ocrBackend
  ...
  const clickExitCode = await run(["clickText", "Message General", "2"], {
    stdout: clickStdout,
    stderr: clickStderr,
    robot,
    ocrBackend
...
test("a quoted query ending in a number remains part of the text", async () => {
  ...
  const exitCode = await run(["clickText", "Version 2"], {
    stdout,
    stderr,
    robot,
    ocrBackend: {
